...
import { IncomingFace, IncomingOtp, IncomingRestriction, OutgoingAccess, User } from './types.js'
...
export function validateUser(user: User): boolean{
  if(user.dateCreated && user.enabled && user.faceDescriptor && user.firstName && user.id && user.lastName && user.role && user.tfaToken
...
export function validateNewUser(user: User): boolean{
  if(user.dateCreated && user.enabled && user.firstName && user.id && user.lastName && user.role && user.tfaToken
    && typeof user.enabled === 'boolean' && typeof user.firstName === 'string' 
    && typeof user.id === 'number' && typeof user.lastName === 'string' && validateRole(user.role as Role) && typeof user.tfaToken === 'string'){
    return true}
  else{
    return false }
...
export function validateIncomingFace(incomingFace: IncomingFace): boolean{ 
  if(validateFaceDescriptor('incomingFace.faceDescriptor')){ // TODO
    return true 
  }
  else {
    return false
